...
class Imager:
    ...
    # file, use the extension JPEG, not JPG, which seems to cause some
    # problems.
    ...
    # The use of Image.eval applies the func to each BAND, independently, if
    # image pixels are RGB tuples.
    def map_image(self, func, image=False):
        ...
        return Imager(image=Image.eval(image, func))
    ...
    def gen_grayscale(
        self, image=False): return self.scale_colors(
        image=image, degree=0)
    ...
    def concat_vert(self, im2=False, background='black'):
        # concat with yourself if no other imager is given.
        im2 = im2 if im2 else self
        im3 = Imager()
    ...
    def morph(self, im2, alpha=0.5):
        ...
        im3 = Imager(width=self.xmax, height=self.ymax)
